...
// A small train-ticket popup shown on hovering a station for a moment:
// origin → destination, line chip, a gallery photo as a stamp, and a
// one-line blurb (station.ticketBlurb — fill in later; falls back to a
// placeholder so the layout always reads right).
...
function ticketPhoto(s) {
  if (s.galleryImages && s.galleryImages[0] && s.galleryImages[0].src) return s.galleryImages[0].src;
  if (s.postcardImage) return s.postcardImage;
  return null;
}

function ticketBlurbText(s) {
  if (s.ticketBlurb) return s.ticketBlurb;
  const c = s.content || {};
  if (c.tagline) return c.tagline;
  if (c.note) return c.note;
  return 'A short description of this stop goes here.';
}

function renderTicket(id) {
  const s = STATIONS[id];
  if (!s) return;
  const line = LINES.find(l => l.id === s.lineIds[0]) || {};
  const lineColor = line.color || 'var(--ink)';
  const origin = (STATIONS[currentStation] && STATIONS[currentStation].name) || 'Central';
  const dest = s.name || '';
  const destSub = s.city || line.city || '';
  const code = (window.STATION_CODES && window.STATION_CODES[id]) || (line.short || '');
  const photo = ticketPhoto(s);
  const blurb = ticketBlurbText(s);
  const sameStop = id === currentStation;

  ticketEl.style.setProperty('--line-color', lineColor);
  ticketEl.innerHTML = `
    <div class="tk-perf tk-perf-top"></div>
    <div class="tk-head">
    ...
    <div class="tk-route">
      ...
      <div class="tk-od tk-od-to">
        ...
        <div class="tk-od-name">${dest}</div>
        <div class="tk-od-sub">${destSub}</div>
      </div>
    </div>
    <div class="tk-body">
      ${photo ? `<div class="tk-stamp"><img src="${photo}" alt="" /><span class="tk-stamp-ring"></span></div>` : ''}
      <p class="tk-blurb">${blurb}</p>
    </div>
    <div class="tk-foot">
      <span>ADMIT ONE</span>
      <span class="tk-foot-code">${code} · ${(line.short || '')}</span>
    </div>
    <div class="tk-perf tk-perf-bottom"></div>`;
}

function positionTicket() {
  const pad = 16;
  const w = ticketEl.offsetWidth || 260;
  const h = ticketEl.offsetHeight || 200;
  let x = ticketPos.x + 20;
  let y = ticketPos.y + 20;
  if (x + w + pad > window.innerWidth)  x = ticketPos.x - w - 20;
  if (y + h + pad > window.innerHeight) y = window.innerHeight - h - pad;
  if (x < pad) x = pad;
  if (y < pad) y = pad;
  ticketEl.style.left = x + 'px';
...
// Build a faint pencil sketch sized to sit behind a station marker on the map.
// Reuses the same STATION_SKETCHES artwork as the panel watermark.
function mapStationSketch(station) {
  const key = station && station.bgSketch;
  const raw = key && window.STATION_SKETCHES && window.STATION_SKETCHES[key];
  if (!raw) return null;
  const place = (window.STATION_SKETCH_PLACEMENT && window.STATION_SKETCH_PLACEMENT[key])
    || { w: 190, h: 320, anchor: 'bottom' };
  const W = place.w, H = place.h;
  // bottom: rises from node · center: sits on node · base: ground rests on the line
  const y0 = place.anchor === 'center' ? -H / 2
           : place.anchor === 'base'   ? -H
           : 36 - H;
  const x = -W / 2 + (place.ox || 0);
  const y = y0 + (place.oy || 0);     // per-sketch nudge to dodge labels
  const inner = raw
    .replace(/pencil-([a-z0-9]+)/g, 'pencil-$1-map')  // unique filter ids vs. the panel copy
    .replace('<svg ', `<svg x="${x}" y="${y}" width="${W}" height="${H}" `);
  const wrap = document.createElementNS(SVG_NS, 'g');
  wrap.setAttribute('class', 'map-sketch');
...
function drawStation(id, wp, station, primaryLine) {
  ...
  g.setAttribute('class', `station station-${id}` + (station.secret ? ' station-secret' : ''));
  g.setAttribute('transform', `translate(${wp.x} ${wp.y})`);
  g.dataset.stationId = id;

  // Faint city/project sketch behind the marker (first child → sits underneath)
  ...
  if (isCentral) {
    ...
    drawSplitDotMarker(g, splitDot);
  } else if (station.secret) {
    drawSecretMarker(g, primaryLine);
  } else if (isMini) {
    drawMiniMarker(g, primaryLine);
  } else if (id === 'education') {
    // Mumbai-style major-station rectangle badge
    drawJourneyMajorMarker(g, STATION_CODES[id]);
  } else if (isTerminus && (primaryLine.id === 'tech-top' || primaryLine.id === 'tech-bottom')) {
    drawTtcTerminusMarker(g, primaryLine);
  } else {
    // ── Per-line default ──
    switch (primaryLine.id) {
      case 'tech-top':
      case 'tech-bottom':
        drawTechStation(g, primaryLine);
        break;
      case 'journey':
        drawJourneyStation(g, primaryLine);
        break;
      case 'creative':
        drawCreativeStation(g, id, primaryLine);
        break;
      case 'founders':
        drawFounderStation(g);
        break;
      default:
        drawDefaultStation(g, primaryLine);
    }
  }
  ...
  g.addEventListener('mouseenter', (e) => {
    ...
    if (station.type === 'central') return;     // skip the hub
...
function drawStationLabel(g, id, station, line) {
  ...
  lbl.setAttribute('text-anchor', labelHints?.anchor || 'start');
  lbl.textContent = station.secret ? '⌁' : station.name;
  ...
  if (isCentral) {
    ...
    startStationNameFlicker(lbl, station.displayNames);
  } else if (isTerminus) {
...
function startStationNameFlicker(labelEl, names = []) {
...
const LABEL_HINTS = {
  ...
  'hackathon-director':   { dx: 0,   dy: -18, anchor: 'middle' },
  'sutd-hackathon':       { dx: -16, dy: 5,   anchor: 'end'    },
  'uwc-hacks':            { dx: 0,   dy: 28,  anchor: 'middle' },  // split dot
  'utra-hacks':           { dx: 0,   dy: 28,  anchor: 'middle' },
  'tides':                { dx: 18,  dy: -10, anchor: 'start'  },
};
...
function createTrain() {
  ...
  // Inner wrapper: flip horizontally (nose to the right) + scale + center on
  // rail-bottom (inner-coords 246, 188 = midpoint between the four wheel bottoms).
  const inner = document.createElementNS('http://www.w3.org/2000/svg', 'g');
  inner.setAttribute('transform', 'scale(-0.38 0.38) translate(-258 -195)');
  inner.innerHTML = TRAIN_INNER_SVG;
  g.appendChild(inner);
  trainNode = g;
  overlayLayer.appendChild(g);
}

function placeTrain(x, y, angleDeg) {
  trainNode.setAttribute('transform', `translate(${x} ${y}) rotate(${angleDeg})`);
}

// Move along a line from lenA to lenB. Returns false if a newer navigation cancels it.
function animateAlong(lineId, lenStart, lenEnd, durationMs, reversed = false, token = navigationToken) {
  return new Promise(resolve => {
    ...
    function step(t) {
      ...
      // tangent for rotation: sample slightly ahead
      const ahead = Math.max(0, Math.min(path.getTotalLength(), len + (lenEnd > lenStart ? 1 : -1)));
      const p2 = path.getPointAtLength(ahead);
      let angle = Math.atan2(p2.y - p.y, p2.x - p.x) * 180 / Math.PI;
      if (reversed) angle += 180;
      placeTrain(p.x, p.y, angle);
...
async function moveTrainTo(targetStationId, token = navigationToken) {
  ...
  for (const leg of route) {
    ...
    const reversed = to.length < from.length;
    // speed: ~700px/sec but with min/max
    const dist = Math.abs(to.length - from.length);
    const dur = Math.max(450, Math.min(2200, dist * 1.6));
    const completed = await animateAlong(lineId, from.length, to.length, dur, reversed, token);
...
function buildRoute(fromId, toId) {
  ...
  // 2) Single transfer through a hub station
  //    central is the main hub; tech-skills is the western Tech terminus
  //    that joins both Tech tracks; hackathon-director / bacsa are stub interchanges.
  const hubs = ['central', 'tech-skills', 'hackathon-director', 'bacsa', 'education'];
...
function placeTrainAtStation(stationId) {
  ...
  if (!path) return;
  const aheadLen = Math.min(path.getTotalLength(), stop.length + 1);
...
function stationSketch(s) {
  const key = s && s.bgSketch;
  const svg = key && window.STATION_SKETCHES && window.STATION_SKETCHES[key];
  if (!svg) return '';
  return `<div class="station-sketch" aria-hidden="true">${svg}</div>`;
}
...
function renderContentPanel(stationId) {
  ...
  const lineText = primaryLine.id.startsWith('tech-')
    ? `${primaryLine.name} (Toronto TTC)`
    : primaryLine.name;

  // Three-column scrapbook layout.
  ...
  const galleryCount = s.galleryCount ?? (galleryImages.length || 3);
  const factText = postcardSlide?.fact || s.funFact || `[Drop in a fun fact about ${s.name} here. Anything memorable — a moment, a lesson, a story.]`;
  const funFactLabel = postcardSlide?.factLabel || s.funFactLabel || 'Fun Fact';
    ...
    <div class="panel-layout">
        ...
        <div class="postcard" style="--line-color:${lineColor}">
          ...
          <div class="postcard-photo-frame">
            <image-slot
              ...
              placeholder="Drop a photo of ${s.name} here"
            ></image-slot>
          </div>
      ...
      <aside class="gallery-col">
        <div class="gallery-label">
          <span>Gallery</span>
          <span class="gallery-count">${galleryCount} slots</span>
        </div>
        <div class="gallery-stack">
          ${Array.from({ length: galleryCount }).map((_, i) => {
            const galleryImage = galleryImages[i] || {};
            const galleryCaption = galleryImage.caption || s.name;
            return `
            <div class="gallery-slot">
              <div class="gallery-slot-frame">
                <image-slot
                  id="gallery-${stationId}-${i + 1}"
                  shape="rect"
                  ${galleryImage.src ? `src="${galleryImage.src}"` : ''}
                  placeholder="Drop image ${i + 1}"
                ></image-slot>
              </div>
              <div class="gallery-slot-caption">
                <span>${galleryCaption}</span>
                <span class="gallery-slot-num">${String(i + 1).padStart(2, '0')}/${String(galleryCount).padStart(2, '0')}</span>
              </div>
            </div>
          `}).join('')}
        </div>
...
function renderProject(c) {
  return `
    <div class="project-card">
      ...
      ${c.url ? `<a class="entry-link" target="_blank" rel="noopener" href="${c.url}">${c.url.replace(/^https?:\/\//,'')} ↗</a>` : ''}
    </div>
  `;
}
...
async function onStationClick(stationId) {
  ...
  if (token === navigationToken) busy = false;
}
